...
def _validate_COMMON_ITERATION_LEVELS(val,errorLog=[]):
  if utilities.is_positive_integer(val) or (val is None):
    ...
    errorLog.append({'Date':datetime.datetime.now(),'Type':'init','Module':'ENERGY',
...
class ENERGY:
  ...
  def SECTION_PARAMETERS(self, val):
    val = str(val).upper()
    if val in BOOL_VALS:
      self.__changeLog.append({'Date': datetime.datetime.now(), 'Module': 'ENERGY', 'Variable': 'SECTION_PARAMETERS',
      ...
      self.__errorLog.append({'Date': datetime.datetime.now(), 'Type': 'Setter', 'Module': 'ENERGY',
                              'Variable': 'SECTION_PARAMETERS', 'ErrorMessage': errorMessage, 'Location': self.__location})

  @ADD_LAST.setter
  def ADD_LAST(self, val):
    self.__changeLog.append({'Date': datetime.datetime.now(), 'Module': 'ENERGY', 'Variable': 'ADD_LAST','Success': True, 'Previous': self.__ADD_LAST, 'New': val,'ErrorMessage': None, 'Location': self.__location})
    self.__ADD_LAST = val

  @COMMON_ITERATION_LEVELS.setter
  def COMMON_ITERATION_LEVELS(self, val):
    if utilities.is_positive_integer(val):
      self.__changeLog.append({'Date': datetime.datetime.now(), 'Module': 'ENERGY', 'Variable': 'COMMON_ITERATION_LEVELS',
  ...
  def LOG_PRINT_KEY(self, val):
    val = str(val).upper()
    if val in BOOL_VALS:
      self.__changeLog.append({'Date': datetime.datetime.now(), 'Module': 'ENERGY', 'Variable': 'LOG_PRINT_KEY',
