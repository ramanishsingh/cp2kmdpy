...
def _validate_SECTION_PARAMETERS(val,errorLog=[]):
  ...
  if val in SECTION_PARAMETERS_VALS or (val is None):
    ...
    errorLog.append({'Date':datetime.datetime.now(),'Type':'init','Module':'PRINT / RESTART_HISTORY',
...
class RESTART_HISTORY:
  ...
  def SECTION_PARAMETERS(self, val):
    ...
    if val in SECTION_PARAMETERS_VALS:
      self.__changeLog.append({'Date': datetime.datetime.now(), 'Module': 'PRINT / RESTART_HISTORY', 'Variable': 'SECTION_PARAMETERS',
      ...
      self.__errorLog.append({'Date': datetime.datetime.now(), 'Type': 'Setter', 'Module': 'PRINT / RESTART_HISTORY',
                              'Variable': 'SECTION_PARAMETERS', 'ErrorMessage': errorMessage,
                              'Location': self.__location})
  ...
  def ADD_LAST(self, val):
    self.__changeLog.append(
      {'Date': datetime.datetime.now(), 'Module': 'PRINT / RESTART_HISTORY', 'Variable': 'ADD_LAST', 'Success': True,
  ...
  def COMMON_ITERATION_LEVELS(self, val):
    if utilities.is_positive_integer(val):
      self.__changeLog.append(
        {'Date': datetime.datetime.now(), 'Module': 'PRINT / RESTART_HISTORY', 'Variable': 'COMMON_ITERATION_LEVELS',
  ...
  def LOG_PRINT_KEY(self, val):
    val = str(val).upper()
    if val in BOOL_VALS:
      self.__changeLog.append({'Date': datetime.datetime.now(), 'Module': 'PRINT / RESTART_HISTORY', 'Variable': 'LOG_PRINT_KEY',
